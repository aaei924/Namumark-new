...
class Namumark
{
    ...
    private $NWPage, $wikitextbox = false, $imageAsLink, $linenotend, $htr, $inlinecomponents;
    ...
    private function renderProcessor($text): array
    {
        ...
        if (str_starts_with($text, '#!html')) {
            ...
            $divattr = '';
            $divdarkattr = '';
            if (preg_match('/(style=\"[^"]*\")([ ]*)?(dark-style=\"([^"]*)\")?/', $match[1], $_dattr)) {
                $divattr = $_dattr[0];
                
                // dark-style
                if (!empty($_dattr[3])) {
                    $divdarkattr = str_replace(';', '!important;', $_dattr[4]);
                    if (!str_ends_with($divdarkattr, ';'))
                        $divdarkattr .= '!important;';
                }
            }
             
            $text = substr($text, strlen($match[0]));
            return [['type' => 'wikitext', 'attr' => $divattr, 'dark-attr' => $divdarkattr, 'text' => $this->blockParser($text, true)]];
